                            ...
                            <Section id='booking-section-1' number='1' title='Service and Pet' description='Select a grooming service and the pet receiving treatment.' icon={<Scissors size={18} className='text-[#2F6B57]' />}>
                                ...
                                <div className='mb-4 flex flex-wrap items-center justify-between gap-3'>
                                    ...
                                    <div className='inline-flex rounded-xl bg-[#EDF3EE] p-1 border border-[#DDE4DE]'>
                                        <button type='button' onClick={() => { setPetMode('existing'); const currentPet = pets.find((p) => p._id === selectedPetId) || pets[0]; resetForPetChange(currentPet) }} disabled={!pets.length} className={`rounded-lg px-4 py-1.5 text-xs font-bold transition-all ${petMode === 'existing' ? 'bg-[#1F4D3E] text-white shadow-xs' : 'text-[#405148] hover:text-[#13231B]'} disabled:opacity-40`}>Saved Pets</button>
                                        <button type='button' onClick={() => { setPetMode('new'); resetForPetChange(newPet) }} className={`rounded-lg px-4 py-1.5 text-xs font-bold transition-all ${petMode === 'new' ? 'bg-[#1F4D3E] text-white shadow-xs' : 'text-[#405148] hover:text-[#13231B]'}`}>New Pet</button>
                                    </div>
                                </div>
                                ...
                                {petMode === 'existing' ? (
                                    <div className='grid gap-3 sm:grid-cols-2'>
                                        {pets.map((pet) => {
                                            ...
                                            return (
                                                <button key={pet._id} type='button' onClick={() => { setSelectedPetId(pet._id); resetForPetChange(pet) }} aria-pressed={selected} className={`flex items-start gap-3 rounded-xl border p-4 text-left transition ${selected ? 'border-[#2F6B57] bg-[#EDF3EE] ring-2 ring-[#2F6B57]/20 shadow-xs' : 'border-[#DDE4DE] bg-white hover:border-[#B8C7BE] hover:shadow-xs'}`}>
